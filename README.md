...
_(Примітка: `sail` — це просто зручна обгортка над `docker compose`)._
...
- **Менеджер логістики** (доступ своєї компанії, створення замовлень): `manager@gmail.com`
- **Спостерігач** (лише перегляд замовлень компанії): `observer@gmail.com`

_Пароль для всіх акаунтів: `password`_
...
- **Document Automation:** Автоматична фонова генерація PDF-документів (CMR, Invoice) із завантаженням у S3 (MinIO) та безпечним контролем доступу (Gate).
